...
${BOLD}Conversion Options:${NC}
  ...
  ${GREEN}--new-api${NC}       Translate JSON "translation" values only (keys untouched)
...
${BOLD}Examples:${NC}
  ...
  $SCRIPT_NAME --new-api -i file.json      Translate translation values only
...
def translate_value(text, sed_script, reverse, opencc_config):
    """Translate a single string value using opencc + sed replacements."""
    if not any('\u4e00' <= c <= '\u9fff' for c in text):
        return text

    if reverse:
        # TW->CN: sed first, then opencc tw2s
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8') as f:
            f.write(text)
            tmp = f.name
        try:
            if sed_script and os.path.getsize(sed_script) > 0:
                subprocess.run(['sed', '-i', '-f', sed_script, tmp], check=True)
            out_tmp = tmp + '.out'
            subprocess.run(['opencc', '-i', tmp, '-o', out_tmp, '-c', opencc_config], check=True)
        ...
        # CN->TW: opencc s2tw first, then sed
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8') as f:
            f.write(text)
            tmp = f.name
        try:
            ...
            subprocess.run(['opencc', '-i', tmp, '-o', out_tmp, '-c', opencc_config], check=True)
            if sed_script and os.path.getsize(sed_script) > 0:
                subprocess.run(['sed', '-i', '-f', sed_script, out_tmp], check=True)
            with open(out_tmp, 'r', encoding='utf-8') as f:
                result = f.read()
            os.unlink(out_tmp)
        finally:
...
def process_new_api(input_path, output_path, sed_script, reverse):
    with open(input_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    opencc_config = 'tw2s' if reverse else 's2tw'

    if 'translation' in data and isinstance(data['translation'], dict):
        new_translation = {}
        for key, value in data['translation'].items():
            if isinstance(value, str):
                new_translation[key] = translate_value(value, sed_script, reverse, opencc_config)
            else:
                new_translation[key] = value
        data['translation'] = new_translation
    else:
        print("Warning: no 'translation' dict found in JSON", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    sed_script = sys.argv[3] if len(sys.argv) > 3 else ''
    reverse = sys.argv[4] == 'true' if len(sys.argv) > 4 else False
    process_new_api(input_path, output_path, sed_script, reverse)
PYTHON_SCRIPT
)

# ------------------------------------------------------------------------------
# New API Processing Function
# ------------------------------------------------------------------------------

process_new_api_json() {
	local input="$1"
	local custom="${2:-}"

	local ext="${input##*.}"
	if [ "$ext" != "json" ] && [ "$ext" != "JSON" ]; then
		log_warn "Skip (not JSON): $input"
		((++total_skipped))
		return 0
	fi
	...
	if [ -n "$custom" ]; then
		output="$custom"
	elif [ "$INPLACE_MODE" = true ]; then
		output="$input"
	else
		output=$(generate_output_filename "$input" "$REVERSE_MODE")
	fi

	[ "$DRY_RUN" = true ] && {
		log_info "${BLUE}[DRY]${NC} new-api: $input -> $output"
		return 0
	}
	...
	[ "$MAKE_BACKUP" = true ] && [ -f "$input" ] && cp "$input" "${input}.bak"

	local sed_script
	[ "$REVERSE_MODE" = true ] && sed_script="$SED_SCRIPT_FILE_REV" || sed_script="$SED_SCRIPT_FILE"

	local reverse_str
	[ "$REVERSE_MODE" = true ] && reverse_str="true" || reverse_str="false"

	local temp_out
	temp_out=$(mktemp --suffix=.json 2>/dev/null || mktemp)

	if python3 -c "$NEW_API_HANDLER" "$input" "$temp_out" "$sed_script" "$reverse_str" 2>/dev/null; then
		if [ "$output" = "$input" ]; then
			mv "$temp_out" "$input"
		else
			local out_dir
			out_dir=$(dirname "$output")
			[ "$out_dir" != "." ] && [ ! -d "$out_dir" ] && mkdir -p "$out_dir"
			mv "$temp_out" "$output"
		fi

		clear_progress
		if [ "$output" = "$input" ]; then
			log_success "Updated (new-api): $input"
		else
			log_success "Wrote (new-api): $input ${BLUE}→${NC} $output"
		fi
		...
		rm -f "$temp_out"
		log_error "Failed (new-api): $input"
		return 1
	fi
}
...
process_amend() {
	...
	[ ! -f "$png_file" ] && {
		log_error "PNG not found: $png_file"
		return 1
	}
	[ ! -f "$json_file" ] && {
		log_error "JSON not found: $json_file"
		return 1
	}
...
# Handle new-api mode.
if [ "$NEW_API_MODE" = true ]; then
	if [ ${#input_args[@]} -eq 0 ]; then
		log_error "Error: --new-api requires at least one JSON file"
		exit 1
	fi

	log_info "${CYAN}Processing ${#input_args[@]} file(s) in new-api mode...${NC}"
	[ "$INPLACE_MODE" = true ] && log_info "${YELLOW}Mode: In-place${NC}"
	[ "$DRY_RUN" = true ] && log_info "${BLUE}Mode: Dry-run${NC}"

	for idx in "${!input_args[@]}"; do
		input="${input_args[$idx]}"
		custom=""
		[ "$CUSTOM_OUTPUT" = true ] && custom="${OUTPUT_FILES[$idx]}"

		if process_new_api_json "$input" "$custom"; then
			((++total_processed))
		else
			((++total_failed))
			failed_files+=("$input")
	...
	echo ""
	log_info "${CYAN}═══════════════════════════════════════${NC}"
	log_info "${CYAN}Summary (new-api)${NC}"
	log_info "${CYAN}═══════════════════════════════════════${NC}"
